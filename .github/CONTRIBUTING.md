...
cd kast
./install.sh --skip-apt   # if you already have the packages in packaging/packages.txt
kast doctor               # confirm the stack is healthy
```

Iterate on `src/bin/kast` directly with `bash src/bin/kast <verb>`, then re-run
...
editing `src/bin/sutra_update.py` in place. It's vendored byte-identical on purpose, and hand
...
in `packaging/shellcheckrc`, not scattered inline. Quote your expansions, and prefer small functions.
...
[docs/USAGE.md](../docs/USAGE.md), `src/data/man/man1/kast.1` if the verb it describes changed, and add a
[docs/CHANGELOG.md](../docs/CHANGELOG.md) entry. Small, focused PRs get looked at fastest.
